/**
 * Created by ZHUNJIEE on 2017/6/29.
 */
...
function bindFace() {
    var html = "";
    html += "<div id='faceImgDiv'>";
    for(var key in userFaces) {
        html += "<img src='images/faces/" + key + "' alt='"+ userFaces[key] + "' />"
    }
    html += "</div>";
    $(html).appendTo($("#funBox .insertFace"));

    $("#faceImgDiv").css("left", $("a.insertFace").offset().left)
        .css("top", $("a.insertFace").offset().top + "px")
        .hide()
...
function bindAtSomeClick() {
    ...
    $.each(friendsList, function (index, ele) {
        html += "<span>@" + friendsList[index] + "</span>";
    });
    html += "</div>";
    ...
    // 点击时
    createDOM.css({
        "left" : $("a.atSome").offset().left + "px",
        "top" : $("a.atSome").offset().top + 18 + "px"
    }).find('span').click(function () {
        console.log($(this).text());
        if ($("#msgTxt").val().match($(this).text()) === null) {
...
function bindTxtChange() {
    ...
    function checkMsgTxt() {
        ...
        if (str.length > 140) {
            ...
            $(".sendBtn").addClass("sendBtnDisabled");
        } else {
            $(".countTxt").html("您还能输入<em>" + (140 - str.length) + "</em>字");
            $(".sendBtn").attr("disabled", false);
            $(".sendBtn").removeClass("sendBtnDisabled");
        }
    }
}
